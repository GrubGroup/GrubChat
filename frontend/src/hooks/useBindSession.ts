...
// works on a COLD entry — a refresh or a pasted /groups/:groupId/sessions/:sessionId
...
export function useBindSession(groupId: number, enabled = true): SessionBinding {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return
      // Nothing to bind — or the session is already CLOSED (the host confirmed a
      // restaurant). The gateway only returns open sessions, but checking closed_at
      // here closes the close-commit race that would otherwise re-arm the inline
      // card for a session that has just ended.
      if (session == null || session.closed_at != null) {

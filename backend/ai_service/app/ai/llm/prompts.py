...
PREFERENCE_TURN_SYSTEM = (
    ...
    "food', 'something Latin', 'European', 'Mediterranean'), record ONLY that "
    "group's canonical group_key (e.g. 'asian', 'latin_american') — do NOT list "
    "its member cuisines. The system expands the key into its members "
    "automatically at search time, so writing them out is wasted, slower output. "
    "Master cuisine groups (group_key => the members it covers, shown only so you "
    "pick the right key — record just the key):\n"
    ...
    "preference — record the single canonical style key in preferred_cuisines "
    "(or disliked_cuisines if they want to avoid that style). Master styles:\n"
    f"{PROMPT_STYLE_CATALOG}\n"
    "Always use the canonical group_key / style key shown above, recorded "
    "COMPACTLY — one key per thing the user named, never its expansion. It is "
    "fine to also include a specific cuisine the user named alongside a group "
    "(e.g. 'Asian, especially ramen' -> asian AND ramen — the group key plus the "
    "one specific cuisine). Never invent a preference the user didn't express.\n\n"
    ...
    "unchanged. A partial turn never nulls an earlier answer. When you carry a "
    "cuisine list through, copy it VERBATIM from CURRENT_SIGNALS — if it already "
    "holds a group key like \"asian\", keep \"asian\", do not expand it into "
    "member cuisines.\n\n"
    ...
    "whole dollars. A lone number is a ceiling -> budget_max. A budget is what "
    "the diner is WILLING TO SPEND UP TO — never a target they want to hit, so "
    "never read a high number as 'wants an expensive place'. A 0 in "
    "CURRENT_SIGNALS means they already declared NO price limit (see below), NOT "
    "a $0 budget — carry it through unchanged unless they name a number now.\n"
    '  "budget_flexible" (bool): true when the user puts NO price limit on this '
    "meal — \"I'm flexible\", \"whatever\", \"price isn't a concern\", \"I don't "
    "care what it costs\", \"money's no object\". This is about MONEY ONLY: a "
    "general \"anything works\" / \"you decide\" about FOOD is "
    "no_cuisine_preference, NOT budget_flexible. When true, leave budget_min and "
    "budget_max null — UNLESS they name a number in the SAME turn (\"I'm "
    "flexible, but let's stay under $30\" -> budget_flexible true AND budget_max "
    "30; the number wins). This ANSWERS the budget question: do not list "
    '"budget" in missing_signals.\n'
...
GROUP_RERANK_SYSTEM = (
    ...
    "restaurants that already passed the HARD filters (dietary needs, distance, "
    "and open-at-the-event-time). PRICE is NOT a hard filter — some candidates "
    "may cost more than some diners want to spend, and it is your job to rank "
    "that down. Rank the candidates for how well they satisfy the WHOLE group, "
    "balancing preferred cuisines (reward), disliked cuisines (penalize), "
    "rating, BUDGET, and LOCATION.\n\n"
    "BUDGET is a per-member CEILING, never a target. Each diner's budget is the "
    "most they are WILLING to spend, so a candidate at or under someone's "
    "ceiling satisfies that diner completely and a CHEAPER candidate is never "
    "worse for anyone. NEVER favor a candidate for being more expensive, and "
    "never treat the group's highest ceiling as an amount to spend up to — a "
    "diner with a high ceiling is expressing tolerance, not a taste for pricier "
    "places. `member_budget_caps` lists each constraining diner's ceiling in "
    "dollars per person (diners who said they're flexible contribute none, so an "
    "empty list means budget is irrelevant here). Each candidate carries "
    "`budget_fit` in [0, 1]: 1.0 means everyone can afford it, and it falls the "
    "further the price runs past the TIGHTEST ceiling in the group. Prefer a "
    "higher `budget_fit` — a spot the whole group can comfortably afford beats a "
    "pricier one with a marginally better cuisine match.\n\n"
    ...
    "tier — between +0.20, host +0.12, member +0.06, far +0.00 — and SUBTRACTS up "
    "to 0.40 x (1 - budget_fit) for price, then re-sorts by the adjusted score. So "
    "a candidate more than ~25% over the group's tightest ceiling loses to an "
    "affordable one even with the best possible location bonus. Rank so your own "
    "order already agrees with that result: reward the proximity tiers and "
    "affordable candidates rather than fighting the adjustment.\n\n"
